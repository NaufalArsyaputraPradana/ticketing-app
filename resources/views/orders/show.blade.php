                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3">
                        ...
                        <div class="lg:col-span-1 p-6 bg-gradient-to-br from-blue-50 to-purple-50">
                            <div class="relative h-64 rounded-lg overflow-hidden mb-4 shadow-md">
                                <img src="{{ $order->event?->gambar ? (filter_var($order->event->gambar, FILTER_VALIDATE_URL) ? $order->event->gambar : asset('images/events/' . $order->event->gambar)) : asset('images/konser.jpeg') }}"
                                    alt="{{ $order->event?->judul ?? 'Event' }}" class="w-full h-full object-cover" />
                            </div>
                            <h2 class="font-bold text-2xl mb-3 text-gray-800">{{ $order->event?->judul ?? 'Event' }}
                            </h2>
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="font-semibold">
                                            {{ $order->event?->waktu->locale('id')->translatedFormat('d F Y, H:i') ?? '-' }}
                                        </p>
                                    </div>
                                </div>
                                    ...
                                    <div
                                        ...
                                        <div class="flex items-center gap-4">
                                            <div class="avatar placeholder">
                                                <div
                                                    class="bg-primary text-white rounded-lg w-16 h-16 flex items-center justify-center">
                                            ...
                                            <div>
                                                ...
                                                <p class="text-sm text-gray-500">@ Rp
                                                    {{ number_format($detail->ticket->harga, 0, ',', '.') }}</p>
                                            </div>
                                        </div>
                                        <div class="text-right">
                                            <p class="text-xs text-gray-500 mb-1">Subtotal</p>
                                            <p class="font-bold text-xl text-primary">Rp
