<x-layouts.admin title="Detail Pemesanan">
    <div class="container mx-auto p-6 lg:p-10">
        ...
        <div class="mb-6">
            ...
            <p class="text-sm text-gray-500 mt-1">Order #{{ $order->id }} •
                ...
                <div class="lg:w-1/3 p-6 border-b lg:border-b-0 lg:border-r border-gray-100">
                    @if ($order->event?->gambar)
                        <div class="rounded-lg border-2 border-gray-200 overflow-hidden shadow-md mb-4">
                            <img src="{{ asset('images/events/' . $order->event->gambar) }}"
                                alt="{{ $order->event->judul }}" class="w-full h-64 object-cover" />
                        </div>
                    @else
                        <div
                            class="w-full h-64 bg-gray-100 rounded-lg border-2 border-gray-200 flex items-center justify-center mb-4">
                    ...
                    <div class="space-y-2">
                        <p class="text-sm text-gray-600 flex items-start gap-2">
                            <svg class="w-5 h-5 flex-shrink-0 mt-0.5 text-primary" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        @if ($order->event?->waktu)
                            <p class="text-sm text-gray-600 flex items-start gap-2">
                                <svg class="w-5 h-5 flex-shrink-0 mt-0.5 text-primary" fill="none"
                    ...
                    <div class="p-4 bg-base-200 rounded-lg border-2 border-gray-200">
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="flex justify-between">
                                ...
                                <span class="text-sm font-medium">{{ $order->created_at->format('d M Y, H:i') }}
                                    WIB</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="space-y-3">
                            @foreach ($order->detailOrders as $detail)
                                <div
                                    class="flex justify-between items-center p-3 bg-base-200 rounded-lg border-2 border-gray-200 hover:border-primary transition-colors">
                                    <div>
                                        <div class="font-bold text-primary">
                                            <span
                                                class="badge badge-primary">{{ ucfirst($detail->ticket->type) }}</span>
                                        </div>
                                        <div class="text-sm text-gray-600 mt-1">
                                            Rp {{ number_format($detail->ticket->harga, 0, ',', '.') }} ×
                                            {{ $detail->jumlah }} tiket
                                        </div>
                                    </div>
